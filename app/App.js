...
class App extends Component {
  ...
  render() {
      let  ticketList = this.props.tickets.filter((ticket) => ticket.segment[0].origin.indexOf(this.props.origin) !== -1 );
...
App.propTypes = {
  ...
  onChooseTicket: PropTypes.func.isRequired,
...
const mapStateToProps = (state) => (
  {
    ...
    tickets: state.tickets
     //.filter((ticket)=>ticket.segment[0].origin === "origin"),
      //.filter((ticket) => ticket.segment[0].origin.indexOf(this.props.origin) !== -1 )
 , }
);
const mapDispatchToProps = (dispatch) => (
  {
   ...
   onChooseTicket: (target, airport) => dispatch(AirportActionCreators.chooseTicket(target, airport))
  }
);
const AppContainer = connect(mapStateToProps, mapDispatchToProps)(App);
render(  <Provider store={aircheapStore}>
    <AppContainer />
  </Provider>,
